...
jQuery(document).ready(function(){
	    jQuery(".seo_manager-tab").hide();
		jQuery("#div-seo_manager-general").show();
	    jQuery(".seo_manager-tab-links").click(function(){
		var divid=jQuery(this).attr("id");
		jQuery(".seo_manager-tab-links").removeClass("active");
		jQuery(".seo_manager-tab").hide();
		jQuery("#"+divid).addClass("active");
		jQuery("#div-"+divid).fadeIn();
		});
});
/* multiple stylish select*/
var $ = {
...
var drop = function(info){var o = {
   ...
   addOption: function(e, element){ 
      ...
      var x = document.getElementById(info.selecteddiv);
	  x.options[index].setAttribute("selected","selected");
   ...
   removeOption: function(e, element){
      ...
      this.selected.forEach(function(select){
         if(select.index == index && !select.removed){
			var x = document.getElementById(info.selecteddiv);
			x.options[select.index].removeAttribute("selected");
            select.removed = true
            select.state = 'remove'
         }
      })
...
//console.log(vals);
// loop for multiselect
jQuery('.multipleselecte').each(function() {
var divid = jQuery(this).attr('id');
vals = [];
var option_all = jQuery("#"+divid+" option:selected").map(function () {
        return vals.push(jQuery(this).index());
    });		
var myDrop =new drop({
   selecteddiv: divid,
   selector:  '#'+divid,
